...
(function () {
    ...
    function initStaticSwalMessages() {
        document.querySelectorAll('[data-mmoney-static-swal-kind]').forEach(function (el) {
            if (el.getAttribute('data-mmoney-swal-shown') === '1') {
                return;
            }

            var kind = el.getAttribute('data-mmoney-static-swal-kind') || 'alert';
            var msgEl = el.querySelector('[data-mmoney-static-swal-message]');
            var msg = msgEl ? msgEl.innerHTML : '';
            if (!msg) {
                return;
            }

            el.setAttribute('data-mmoney-swal-shown', '1');

            if (kind === 'error' && typeof LibMessageError === 'function') {
                LibMessageError('Atenção', msg);
            } else if (typeof LibMessageAlert === 'function') {
